...
def fps_query(ref_points, k):
    # import pdb;pdb.set_trace()
    return furthest_point_sample(ref_points, k).long()
...
class Net(nn.Module):
    ...
    def calc_ppf(self, pts, normal):
        ...
        # import pdb;pdb.set_trace()
        # fps_index_list = fps_query(pts, 300)[:,None,:].repeat(1,n,1)
        # import pdb;pdb.set_trace()
        patches_list = []
        ...
        # patches_list.append(pts[torch.arange(b)[:,None,None], fps_index_list])
        # patches_normals_list.append(normal[torch.arange(b)[:,None,None], fps_index_list])
        ppfs = []
    ...
    def inference(self,inputs):
        ...
        pts= inputs['pts']
        rgb, _ = inputs['rgb'][:,0,:,:], inputs['rgb'][:,1,:,:]
        pts, _ = inputs['pts'][:,0,:,:], inputs['pts'][:,1,:,:]
        ...
        rgb_raw, _ = inputs['rgb_raw'][:,0,:,:], inputs['rgb_raw'][:,1,:,:]
        feature = self.extract_feature(rgb_raw).reshape(b,(self.num_patches)**2,-1)

        match_num = 100
        choose = inputs['choose'][:,0,:match_num]
        ...
        fps_index = fps_query(pts.contiguous(), 300)
        
        ptsg = pts[torch.arange(b)[:,None], fps_index, :]
        # import pdb;pdb.set_trace()
        with torch.no_grad():
        ...
        # backbone
        # x = self.spherical_fpn(dis_map, torch.cat([rgb_map,ppf_map],dim = 1))
    ...
    def forward(self, inputs):
        ...
        #         ptsg = pts[:, pts.shape[1]//splits * i:pts.shape[1]//splits * (i+1), :]
        #         normals.append(self.cal_normal(ptsg, inputs['rand_rotation'], 
        #                                        inputs['translation_label'], inputs['size_label']))
        #         # ppf_feature = self.calc_ppf(ptsg, normals)
        #         # ppf_features.append(ppf_feature)
        #     # ppf_features = torch.cat(ppf_features, dim = 1)
        #     normals = torch.cat(normals, dim = 1)
        #     import pdb;pdb.set_trace()
        #     ppf_features.append(self.calc_ppf(pts, normals))
        # ppf_feature = torch.cat(ppf_features, dim = 1)
        # import pdb;pdb.set_trace()
        
        fps_index = fps_query(pts, 300)
