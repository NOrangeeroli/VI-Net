...
import math, open3d
import numpy as np
from module import SphericalFPNL, V_Branch, I_Branch, I_Branch_Pair
...
class Net(nn.Module):
    def __init__(self, resolution=64, ds_rate=2, num_patches = 15):
        ...
        self.spherical_fpn = SphericalFPNL(ds_rate=self.ds_rate, dim_in1=1, dim_in2_list=[3,4* len(self.ppf_nn)])#387)
    ...
    def calc_ppf(self, pts, normal):
        b,n,_ = pts.shape
        k_list = self.ppf_nn
        knn_index_list = knn_query_list(pts, pts, k_list)
        patches_list = []
    ...
    def inference(self,inputs):
        ...
        x = self.spherical_fpn(dis_map, [rgb_map,ppf_map])
        # viewpoint rotation
