...
class TrainingDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.mode =='ts':
            ...
            while True:
                pair_index = self.instance_index[index]
                data_dict = self._read_pair(pair_index)
                #print('READ DATA',index,"/",self.__len__(),time.time()-st)
                if data_dict is None:
                    index = np.random.randint(self.__len__())
                    continue
    ...
    def _load_data(self,img_path,img_type, cam_cx, cam_cy,cam_fx, cam_fy, instance_id = -1, without_noise = False):
        ...
        pts2_raw = (depth.copy()[rmin:rmax, cmin:cmax]).reshape((-1))/ self.norm_scale
        pts0_raw = (self.xmap[rmin:rmax, cmin:cmax].reshape((-1)) - cam_cx) * pts2_raw / cam_fx
        pts1_raw = (self.ymap[rmin:rmax, cmin:cmax].reshape((-1))- cam_cy) * pts2_raw / cam_fy
        ...
        if self.mode == 'ts':
            ...
            # assert pts_raw.shape == torch.Size([self.num_patches, self.num_patches, 3])
    ...
    def _read_instance_from_category_dict(self, cls, idx ):
        ...
        pts, rgb, translation, rotation, size, cat_id, asym_flag, \
            choose, rgb_raw, pts_raw, mask = tuple_instance
...
class TestDataset():
    ...
    def _get_pair_random(self,index):
        ...
        for rank, cls in enumerate(ret_dict['pred_class_ids']):
            ...
            reference_rgb_raw.append((rank,refer_dict['rgb_raw']))
            reference_pts_raw.append((rank,refer_dict['pts_raw']))
            reference_choose.append((rank,refer_dict['choose']))
